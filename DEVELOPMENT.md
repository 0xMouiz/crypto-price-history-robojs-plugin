# 🛠️ Developing crypto-price-history
...
```bash
npm install /path/to/crypto-price-history
```

Replace `/path/to/crypto-price-history` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `npm run build` command.
